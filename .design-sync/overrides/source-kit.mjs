...
export async function resolvePackage(ctx) {
  ...
  if (srcRoot) {
    for (const c of components) {
      ...
      // FORK (see cfg.libOverrides): upstream derives the group from the src
      // directory. This repo is a Next.js app whose src/ is organized by
      // application feature (home/, ris/, features/impact/, features/auth/)
      // and by an internal barrel dir (components/rfds/) — none of which is a
      // design-system taxonomy. Upstream's dir-derived value is also
      // load-bearing: package-build only lets a docs `category:` override a
      // group that is falsy/'general'/'misc', so 'home' and 'rfds' silently
      // beat the curated categories and produced group names like "rfds"
      // (meaningless — the whole DS is RFDS) and "auth".
      //
      // So: don't guess a group from the layout. Leave it 'general' and let
      // the curated `category:` frontmatter in cfg.docsDir decide, with JSDoc
      // @category as the fallback. src enrichment (srcPath, JSDoc) is kept.
      // .design-sync/gen-docs.mjs warns on any roster component it does not
      // assign, so a missing category surfaces instead of silently landing
      // in 'general'.
